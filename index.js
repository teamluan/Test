  ...
  try {
      ...
      if (interaction.commandName === 'ausnach') {
        ...
        for (let i = 1; i <= 5; i++) {
          ...
          if ((label && !response) || (!label && response)) {
            return interaction.reply({ content: `❌ Option ${i}: beide Felder müssen zusammen angegeben werden.`, ephemeral: true });
          }
    ...
    if (interaction.isStringSelectMenu()) {
      if (interaction.customId === 'setup_command') {
        if (interaction.user.id !== SETUP_USER_ID) {
          return interaction.reply({ content: '❌ Du darfst das Setup nicht benutzen.', ephemeral: true });
        }
        ...
        if (!roles.length) {
          return interaction.update({ content: '❌ Es wurden keine auswählbaren Rollen gefunden.', embeds: [], components: [] });
        }

        const roleMenu = new StringSelectMenuBuilder()
          .setCustomId(`setup_role:${commandName}`)
          .setPlaceholder('Rolle auswählen ...')
          .addOptions(roles.map(role => ({
            label: role.name.slice(0, 100),
            value: role.id,
            description: `Darf /${commandName} benutzen`
          })));

        return interaction.update({
          content: `⚙️ Rolle für **/${commandName}** auswählen:`,
          embeds: [],
      ...
      if (interaction.customId.startsWith('setup_role:')) {
        ...
        return interaction.update({
          content: `✅ **/${commandName}** darf jetzt die Rolle **${role?.name ?? 'Unbekannt'}** benutzen.\n\nDu kannst den Command weiterhin benutzen.`,
          embeds: [],
          components: []
        });
      }
    ...
    if (!interaction.replied && !interaction.deferred) {
      await interaction.reply({ content: '❌ Bei der Interaktion ist ein Fehler aufgetreten.', ephemeral: true }).catch(() => {});
    }
  }
});
